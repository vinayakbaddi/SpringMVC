...
import com.way2learnonline.repository.CustomClusterRepository;

@SpringBootApplication
public class TestRepositoryDefinition {

	public static void main(String[] args) {
		
		
		ApplicationContext context= new AnnotationConfigApplicationContext(TestRepositoryDefinition.class);
		
		CustomClusterRepository clusterRepository=context.getBean(CustomClusterRepository.class);
		
		Cluster cluster=createCluster();
		
		clusterRepository.save(cluster);
		 
		cluster=clusterRepository.findOne(new Long(1));
		...
		//clusterManager.createServer(firstServer);
